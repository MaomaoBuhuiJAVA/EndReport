...
const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    float sweep = sin((p.x + p.y * 0.35) * 7.0 - uTime * 1.35) * 0.5 + 0.5;
    float edge = smoothstep(0.72, 0.08, length(vec2(p.x * 0.55, p.y)));
    float streak = smoothstep(0.84, 1.0, sweep) * edge;
    float glint = smoothstep(0.97, 1.0, sin((p.x - p.y) * 15.0 + uTime * 0.8));
    vec3 color = vec3(0.95, 1.0, 1.0) * (streak * 0.22 + glint * 0.08);
    gl_FragColor = vec4(color, edge * (0.16 + streak * 0.36));
  }
`;

export function SpecularButton({ children, icon, tone = 'teal', className = '', ...props }: SpecularButtonProps) {
  ...
  return (
    <button ref={hostRef} className={`specular-button specular-button--${tone} ${className}`} {...props}>
      <canvas ref={canvasRef} className="specular-button__canvas" aria-hidden="true" />
      <span className="specular-button__content">
        {icon && <span className="specular-button__icon">{icon}</span>}
        <span>{children}</span>
      </span>
    </button>
  );
